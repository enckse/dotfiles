...
return {
    request = function(args)
        ...
        local check_file = function()
            if not ioutils.execute(string.format("file '%s' | grep -q '%s'", file, file_type)) then
            ...
            if not ioutils.execute(string.format("%s | grep -q '^%s$'", hash_command, use_hash)) then
        ...
        if ioutils.execute(string.format("test -e '%s'", file)) then
            if check_file() then
                return
            end
        end
        utils.log(string.format("downloading: %s", args.url))
        if not ioutils.execute(string.format("curl --silent -L '%s' > '%s'", args.url, file)) then
