...
mappers.nop = function(shortcuts)
	for _, val in pairs(shortcuts) do
		for _, fxn in pairs({ mappers.nmap, mappers.imap, mappers.vmap }) do
			fxn(val, "<Nop>")
		end
	end
end
...
mappers.nop({ "<S-Up>", "<S-Down>", "<S-Right>", "<S-Left>", "<C-h>", "<C-l>", "<C-q>" })
